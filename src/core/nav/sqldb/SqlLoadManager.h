...
public:
    ...
    std::shared_ptr<navdb::NavDatabase> getWorld() override { return sqlworld; }
    ...
    std::vector<std::shared_ptr<navdb::Airport>> getMatchingAirports(const std::string &pattern);
    std::shared_ptr<navdb::Airport> getAirport(const std::string &id);
    std::shared_ptr<world::Fix> getFix(const std::string &region, const std::string &id);

    navdb::NodeList getFixList(const std::vector<int> &fixKeys);
