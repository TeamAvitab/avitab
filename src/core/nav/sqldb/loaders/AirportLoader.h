...
public:
    ...
    std::shared_ptr<navdb::Airport> load(std::vector<std::shared_ptr<world::Fix>> *ils_fixes = nullptr);
...
private:
    ...
    std::shared_ptr<navdb::Airport> a;
    int airport_id;
    std::string ident;
    std::string region;
    std::map<int, std::shared_ptr<navdb::Runway>> rws;
};
