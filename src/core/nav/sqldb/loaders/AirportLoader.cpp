...
std::shared_ptr<navdb::Airport> AirportLoader::load(std::vector<std::shared_ptr<world::Fix>> *ils_fixes)
{
    ...
    a = std::make_shared<navdb::Airport>(ident);
    a->setName(name);
    a->setRegion(r);
...
inline navdb::ATCserviceType mapToATCclass(const std::string &type) {
    // LNM populates its MSFS DB with these comms tags: A C D G T UC MC CPT CTR FSS RCD ATIS ASOS AWOS CTAF
    if (type.size() == 1) {
        switch (type[0]) {
            case 'A': return navdb::ATCserviceType::APP;
            case 'C': return navdb::ATCserviceType::CLD;
            case 'D': return navdb::ATCserviceType::DEP;
            case 'G': return navdb::ATCserviceType::GND;
            case 'T': return navdb::ATCserviceType::TWR;
            default:  return navdb::ATCserviceType::RECORDED;
        };
    }
    if (type.size() == 2) {
        switch (type[0]) {
            case 'U': return navdb::ATCserviceType::UNICOM;
            case 'M': return navdb::ATCserviceType::MULTICOM;
            default:  return navdb::ATCserviceType::RECORDED;
        };
    }
    if (type.size() == 3) {
        switch (type[1]) {
            case 'P': return navdb::ATCserviceType::CLD;         // CPT
            case 'T': return navdb::ATCserviceType::CTR;         // CTR
            case 'S': return navdb::ATCserviceType::FSS;         // FSS
            default:  return navdb::ATCserviceType::RECORDED;
        };
    }
    return navdb::ATCserviceType::RECORDED;
}
...
void AirportLoader::addComms()
{
    ...
    while (1) {
        ...
        auto f = q->getInt(1);
        auto name = q->getString(2);
        navdb::RadioFrequency frequency(f, 6, navdb::RadioFrequency::Unit::MHZ, name);
        a->addATCFrequency(mapToATCclass(type), frequency);
    }
}

inline navdb::Runway::SurfaceMaterial mapToSurfaceMaterial(const std::string &rwy) {
    // LNM populates its MSFS DB with these surface tags: A B C CE CR D G GR I M OT S SN T UNKNOWN W
    if (rwy.size() == 1) {
        switch (rwy[0]) {
            case 'A': return navdb::Runway::SurfaceMaterial::ASPHALT;
            case 'B': return navdb::Runway::SurfaceMaterial::BITUMINOUS;
            case 'D': return navdb::Runway::SurfaceMaterial::DIRT;
            case 'G': return navdb::Runway::SurfaceMaterial::GRASS;
            case 'I': return navdb::Runway::SurfaceMaterial::ICE;
            case 'M': return navdb::Runway::SurfaceMaterial::TARMAC;
            case 'S': return navdb::Runway::SurfaceMaterial::SAND;
            case 'T': return navdb::Runway::SurfaceMaterial::TARMAC;
            case 'W': return navdb::Runway::SurfaceMaterial::WATER;
            default:  return navdb::Runway::SurfaceMaterial::UNKNOWN;
        };
    }
    if (rwy.size() >= 2) {
        switch (rwy[0]) {
            case 'C': switch (rwy[1]) {
                case 'E': return navdb::Runway::SurfaceMaterial::CONCRETE;
                case 'R': return navdb::Runway::SurfaceMaterial::CORAL;
                default:  return navdb::Runway::SurfaceMaterial::UNKNOWN;
            }
            case 'G': return navdb::Runway::SurfaceMaterial::GRAVEL;
            case 'S': return navdb::Runway::SurfaceMaterial::SNOW;
            default:  return navdb::Runway::SurfaceMaterial::UNKNOWN;
        }
    }
    return navdb::Runway::SurfaceMaterial::UNKNOWN;
}
...
void AirportLoader::addRunways()
{
    ...
    struct RunwayPair {
        ...
        RunwayPair(std::shared_ptr<navdb::Runway> r, float o) : n(1), forward(r), offset_sum(o) { }
        void AddReverse(std::shared_ptr<navdb::Runway> r, float o) { ++n; reverse = r; offset_sum += o; }
        int n;
        std::shared_ptr<navdb::Runway> forward;
        std::shared_ptr<navdb::Runway> reverse;
        float offset_sum;
    };
    ...
    while (1) {
        ...
        auto r = std::make_shared<navdb::Runway>(name);
...
void AirportLoader::addHeliports()
{
    ...
    while (1) {
        ...
        auto h = std::make_shared<navdb::Heliport>(name.str());
        h->setLocation(loc);
        a->addHeliport(h);
    }
}
...
void AirportLoader::addLocalizers(std::vector<std::shared_ptr<world::Fix>> *fixes)
{
    ...
    while (1) {
        ...
        navdb::RadioFrequency ilsFrq(freq, 3, navdb::RadioFrequency::Unit::MHZ, description);
