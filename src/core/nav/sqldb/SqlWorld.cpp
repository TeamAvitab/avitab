    ...
    for (auto outer: visitOrder) {
        for (auto area: outer) {
            ...
            for (auto node: nit->second) {
                ...
                bool accept = false;
                if (node->isAirport()) {
                    if (std::dynamic_pointer_cast<navdb::Airport>(node)->hasControlTower()) {
                        accept = (filter & navdb::NavDatabase::VISIT_TOWERED_AIRPORTS);
                    } else {
                        accept = (filter & navdb::NavDatabase::VISIT_OTHER_AIRPORTS);
                    }
                } else if (node->isFix()) {
                    auto f = std::dynamic_pointer_cast<world::Fix>(node);
                    if (f->isNavaid()) {
                        accept = (filter & navdb::NavDatabase::VISIT_NAVAIDS);
                    } else if (f->isUserFix()) {
                        accept = (filter & navdb::NavDatabase::VISIT_USER_FIXES);
                    } else {
                        accept = (filter & navdb::NavDatabase::VISIT_FIXES);
                    }
                }
...
std::shared_ptr<navdb::Airport> SqlWorld::findAirportByID(const std::string &id) const
{
...
std::shared_ptr<navdb::Airport> SqlWorld::findAirportByCode(const std::string &id) const
{
    std::string cleanId = platform::upper(id);
    cleanId.erase(std::remove(cleanId.begin(), cleanId.end(), ' '), cleanId.end());
    return loadManager.lock()->getAirport(cleanId);
...
std::vector<std::shared_ptr<navdb::Airport>> SqlWorld::findAirport(const std::string &keyWord) const
{
    return loadManager.lock()->getMatchingAirports(keyWord);
}

std::vector<navdb::NavDatabase::Connection> &SqlWorld::getConnections(std::shared_ptr<navdb::Node> from)
{
    return noConnection;
}

bool SqlWorld::areConnected(std::shared_ptr<navdb::Node> from, const std::shared_ptr<navdb::Node> to)
{
    return false;
}
...
void SqlWorld::addAirport(std::shared_ptr<navdb::Airport> a)
{
...
void SqlWorld::addNodeToArea(int lonx_idx, int laty_idx, std::shared_ptr<navdb::Node> node)
{
    std::lock_guard<std::mutex> guard(navStateGuard);
    auto area = std::make_pair(lonx_idx, laty_idx);
    auto it = areaNodes.find(area);
    if (areaNodes.find(area) == areaNodes.end()) {
        areaNodes[area] = std::vector<std::shared_ptr<navdb::Node>>();
