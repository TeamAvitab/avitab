...
void UserFixParser::parseLine() {
    ...
    if (parseType(typeString) == UserFixType::NONE) {
        lineNum++;
        return;
    }
    UserFixData userFix {};
...
UserFixType UserFixParser::parseType(std::string& typeString) {
    if ((typeString == "VRP") || (typeString == "11")) {
        return UserFixType::VRP;
    } else if ((typeString == "POI") || (typeString == "8")) {
        return UserFixType::POI;
    } else if ((typeString == "Marker") || (typeString == "9") || (typeString == "10")) {
        return UserFixType::MARKER;
    } else {
        return UserFixType::NONE;
    }
}
